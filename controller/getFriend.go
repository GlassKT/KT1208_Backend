...
func GetFriend(c *gin.Context) {
	...
	if err := database.DB.Where("user = ? and status = ?", user, "allow").Find(&friend).Error; err != nil { // 받은 id중 status가 wait인 것을 추출
	...
	hobby := modules.GetHobbySlice(getFriendInfo) // 유저마다 hobby를 불러옴
